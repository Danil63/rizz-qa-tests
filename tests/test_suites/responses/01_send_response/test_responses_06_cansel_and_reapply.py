"""responses-06-cpm-cancel-and-reapply: Отмена и повторная отправка отклика за просмотры (CPM)."""
...
@allure.story("Повторная отправка отклика за просмотры (CPM)")
@allure.tag("Regression", "Responses", "CPM", "CancelAndReapply")
class TestResponsesSend06CPMCancelAndReapply:
    @allure.title(
        "responses-send-06: creator market → cancel → reapply CPM response"
    )
    @allure.severity(allure.severity_level.CRITICAL)
    def test_cancel_and_reapply_response_cpm(self, blogger_page: Page):
        ...
        # 6) Нажать "Начать сразу за ..." (цена рассчитывается динамически) + проверка закрытия модалки
