...
class IntegrationPage(BasePage):
    ...
    def __init__(self, page: Page):
        ...
        # ── Кнопки "Отправить" (type=submit) ─────────────────
        self.submit_buttons = page.locator("button[type='submit']", has_text="Отправить")
    ...
    def upload_media_and_submit(self, step_index: int, file_path: str = TEST_IMAGE_PATH) -> None:
        ...
        # Ожидание загрузки — появление превью изображения внутри шага
        self.page.wait_for_timeout(2000)

        submit_btn = self.submit_buttons.nth(step_index)
        expect(submit_btn).to_be_visible(timeout=10000)
        expect(submit_btn).to_be_enabled(timeout=10000)
        submit_btn.click()

        # Проверка: кнопка "Отправить" пропала (шаг завершён)
